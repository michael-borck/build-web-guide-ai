...
<body>
    ...
    <script>
        async function loadPosts() {
            ...
            try {
                const API = 'http://your-site.local';
                const response = await fetch(
                    `${API}/wp-json/wp/v2/posts?_embed&per_page=5`
                );
...
This develops **making and defending technology choices** and **assessing emerging technologies**. Headless architecture is a significant trend in web development. Understanding when it applies, and when it doesn't, demonstrates professional judgment.
:::
