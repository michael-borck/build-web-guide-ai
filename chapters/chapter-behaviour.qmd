...
::: {.callout-tip title="Your Most Important Tool: Browser Developer Console"}
Every modern browser has a built-in developer console. Open it with **F12** (or right-click any page and select **Inspect**). It has three tabs you will use constantly:

- **Elements**: see the actual HTML and CSS the browser received. Useful for checking whether AI-generated markup is structured correctly, or why a style is not applying.
- **Console**: see `console.log()` output, JavaScript errors, and warnings. This is where you debug behaviour.
- **Network**: see every file and API request the page makes. Useful for debugging fetch calls and checking what data comes back.

Get comfortable here. When AI gives you code that does not work, the dev console tells you *why*. It shows you what the browser actually received, not what you intended to send.
:::
...
This develops **evaluating and improving solutions** and **translating stakeholder needs into technical requirements**. Interactive features must serve user needs. JavaScript for its own sake creates complexity without value. The ability to identify when interactivity helps versus hinders is professional judgment.
:::
